...
#from Test_PDF import * # for global variable gui
import time
...
class PDF(FPDF):
        def header(self):
                ...
                #Title
                self.cell(w,9,self.title,0,0,'C',0)
                #Line break
                self.ln(10)
	...
	def complete_list(self,lists):
        ...
        def print_chapter(self):
                self.add_page()
                self.chapter_body()
        def setdata(self,result,ExeTime,Processortime,title,author):
                self.result = result
